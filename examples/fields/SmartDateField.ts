    ...
    const testEgressCycle = (d: Date) => {
      try {
        ...
        if (!verifyEgressCycle(f, d)) {
          egressDebug(f, d)
          throw new Error(`Error: instantiating a SmartDateField with an formatString of ${formatString}, and locale of '${locale}'.  will result in data loss or unexpected behavior`)
        }
      } catch (e: any) {
        egressDebug(f, d)
        throw new Error(`Error: instantiating a SmartDateField with an formatString of ${formatString}, and locale of '${locale}'.  will result in data loss or unexpected behavior`)
